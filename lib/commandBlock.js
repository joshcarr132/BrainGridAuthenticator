...
function commandBlock(client, blockId, blockTime = 8000) {
  return new Promise((resolve, reject) => {
    let blockData = {
      blockId: blockId,
      commands: {}
    }; // a log to store all command data from the block
    ...
    client.createSession({ _auth: client._auth, status: 'open' })
      ...
      .then(subs => {
        ...
        setTimeout(() => { // once command block initialized, start timer which will end the block
          ...
          // determine command by count
          // let highestCount = [];
          // for (let command in blockData.commands) {
          //   let count = blockData.commands[command][0];
          //   if (!highestCount[0]) { highestCount = [command, count] }
          //   else if (count > highestCount[1]) { highestCount = [command, count] }
          // }
          // client._log(`Command: ${highestCount[0]}  |  count: ${highestCount[1]}`);
